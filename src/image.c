...
//     }
//     printf("err code : %d w = %d h = %d \n", err, width, height);

//     if(err != 0) goto end;
//     sImg* image = malloc(sizeof(sImg)+(sizeof(char)*width*height));
//     if(!image) goto end;
    
//     image->height = height;
//     image->width = width;
//     char img_data[320 * 240];

//     fgets(img_data, sizeof(img_data), fp);
//     strcpy(image->data, img_data);
//     for (size_t i = 0; i < (image->width * image->height) ; i++)
//     {
//         image->data[i] = (image->data[i] < 127) ? 0 : 255;
//     }
    
//     printf("%s", image->data);
//     printf("copying image %d %d\n", width, height);
//     FILE *fptr = fopen("sample.pgm", "w");
//     fprintf(fptr, "P5\n%d %d\n255\n", image->width, image->height);
//     fwrite(image->data, sizeof(unsigned char), image->width*image->height, fptr);
//     fclose(fptr);
